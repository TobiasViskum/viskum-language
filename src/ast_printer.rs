...
impl ExprVisitor<Output> for AstPrinter {
    ...
    // These were generated by copilot. They should be verified.
    fn visit_variable_expr(&self, expr: &VariableExpr) -> Result<Output, ViskumError> {
        Ok(format!("(var {})", expr.token.lexeme))
    }

    fn visit_assign_expr(&self, expr: &AssignExpr) -> Result<Output, ViskumError> {
        Ok(format!("(assign {} {})", expr.token.lexeme, expr.value.accept(self)?))
    }
    ...
    fn visit_call_expr(&self, expr: &CallExpr) -> Result<Output, ViskumError> {
        let mut str_builder = format!("(call {} ", expr.callee.accept(self)?);

        for arg in &expr.arguments {
            str_builder = format!("{str_builder} {}", arg.accept(self)?);
        }
